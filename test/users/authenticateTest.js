...
describe('Authenticate users tests', () => {
  it('Should validate the input', async () => {
    const authUserEvent = buildEvent(credentials(buildUser(uniqueEmail())))
  ...
  it('Should not authenticate user with wrong password and increase the wrong attempts counter', async () => {
    let user = buildUser(uniqueEmail())
    const createUserEvent = buildEvent(user)
    ...
    notAuthorizedUser.should.have.property('loginData')
    notAuthorizedUser.loginData.should.have.property('wrongAttempts').which.is.equal(1)
    await testError(authUserFunc, authUserEvent, 401, errorsNumber.invalidPassword)
    notAuthorizedUser = await userDb.getByEmail(user.email)
    notAuthorizedUser.loginData.should.have.property('wrongAttempts').which.is.equal(2)
  })
  ...
  it('Should eliminate wrong attempts data and authenticate the user', async () => {
    ...
    authorizedUser.should.have.property('loginData')
    authorizedUser.loginData.should.have.property('wrongAttempts').which.is.equal(0)
    authorizedUser.loginData.should.have.property('lastLoginAt')
  })
})
